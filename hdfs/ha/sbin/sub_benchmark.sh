...
do
    ...
    if [ $? -ne 0 ]; then
        echo "${ERRORS[$FATAL]}[sub_benchmark:get_failure]: get hdfs file faield"
        exit 1
    else
    ...
    if [ $? -ne 0 ]; then
        echo "${ERRORS[$FATAL]}[sub_benchmark:remove_failure]: remove hdfs file faield"
        exit 1
    else
    ...
    if [ $? -ne 0 ]; then
        echo "${ERRORS[$FATAL]}[sub_benchmark:put_failure]: put hdfs file faield"
        exit 1
    else
